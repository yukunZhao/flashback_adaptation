...
def create_ref_model(
    model_args: "ModelArguments", finetuning_args: "FinetuningArguments", add_valuehead=False,  only_load_embeds=False
    ...
    if finetuning_args.ref_model is not None:
        ...
        ref_model = load_model(
            tokenizer, ref_model_args, ref_finetuning_args, is_trainable=False, add_valuehead=add_valuehead, only_load_embeds=only_load_embeds
        )
...
def get_batch_logps(
    ...
    logits = logits[:, :-1, :] # bsz, seq_len, vocab_size
    loss_mask = labels != label_pad_token_id
    labels[labels == label_pad_token_id] = 0  # dummy token
    per_token_logps = torch.gather(logits.log_softmax(-1), dim=2, index=labels.unsqueeze(2)).squeeze(2)
    return (per_token_logps * loss_mask).sum(-1), loss_mask.sum(-1)

def get_batch_logits_distance():
    return None