...
class Trainer:
    ...
    def _retrieve_grad(self, model):
        grad, shape, has_grad = [], [], []
        # [p.grad.shape for p in model.parameters() if p.grad is not None]
    ...
    def _project_conflicting(self, grads, has_grads, shapes=None):
        ...
        # print ('debug _project_conflicting', len(pc_grad), pc_grad[0].shape)

        # pc grad 是一个 2*
        merged_grad = torch.mean(torch.stack(pc_grad), dim=0).to(grads[0].device)

        # print ('[zhaoyukun] merge_grad.shape', merged_grad.shape)
    ...
    def pcgrading(self, grads, shapes):
        """
        Apply PCGrad to resolve conflicting gradients.
        Args:
            grads (list of tensors): Gradients for each task.
        Returns:
            list of tensors: Adjusted gradients after applying PCGrad.
        """
        num_tasks = len(grads)
        # pcgrads = []
        # for i in range(num_tasks):
        #     pcgrads.append(g.clone() for g in grads[i])

        pcgrads = [g.clone() for g in grads]

        for i in range(num_tasks):
            for j in range(num_tasks):
                if i != j:
                    dot_product = torch.dot(grads[i].view(-1), grads[j].view(-1))
                    if dot_product < 0:
                        pcgrads[i] -= (dot_product / grads[j].norm()**2) * grads[j]
        
        # 两个task 需要merge
        merged_grad = torch.sum(pcgrads, dim=0, keepdim=True).to(grads[0].device)

        # print (f"[zhaoykun merged grad done] merged_grad.shape={merged_grad.shape}")
    ...
    def _inner_training_loop(
        ...
        logger.info("-- Use zhaoyukun02 training ")
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for _ in range(total_updates):
                ...
                # batch_samples 
                #print (f"[zhaoyukun trainer.py] total_updates={total_updates}, batch_samples_len={len(batch_samples)} steps_in_epoch={steps_in_epoch}")
                task_grads = []
                task_grad_shapes = []
                has_grads = []
                for inputs in batch_samples:
                    ...
                    # zhaoyukun02 step1 grad regularization code init
                    grad_ind, shape_ind, has_grad_ind = self._retrieve_grad(model)

                    # grads_ind 是个list， 448 * tensor([8, 4096])
                    task_grads.append(self._flatten_grad(grad_ind, shape_ind))
                    has_grads.append(self._flatten_grad(has_grad_ind, shape_ind))
                    task_grad_shapes.append(shape_ind)
   
                    # print (f"zhaoyukun debug grads.shape={len(grad_ind)}, {grad_ind[0].shape}")
                    # print (f"zhaoyukun debug task_grads.shape={task_grads[0].shape}") # ([20971520])
                    #print (f"zhaoyukun debug grads=f{grads_yk}")
                    # code done
                    ...
                    if do_sync_step:
                        ...
                        # zhaoyukun02
                        #print (f"zhaoyukun step2, do sync, len(task_grads)={len(task_grads)}")
                        pcgrads = self._project_conflicting(task_grads, has_grads)
                        # pcgrads = self.pcgrading(task_grads, task_grad_shapes) 
                        pcgrads = self._unflatten_grad(pcgrads, task_grad_shapes[0])
                        #print (f"zhaoyukun, do sync final done, len(pcgrads), shape)={len(pcgrads)}, {pcgrads[0].shape}")

                        # Update model gradients with PCGrad results
                        idx = 0
                        for param in model.parameters():
                            if param.grad is not None:
                                #print ('[zhaoyukun upd para]', param.grad.shape, pcgrads[idx].shape)
                                param.grad = pcgrads[idx].clone()
                                idx += 1
                            # else:
                            #     param.grad.copy_(g)
                        
                        #print ('do sync succeed')
                        task_grads = []
                        task_grad_shapes = []
                        has_grads = []
                        ...
                        if optimizer_was_run:
                            ...
                            # loggger.info("zhaoyukun02, optimizer_was_run")
    ...
    def training_step(
        ...
        # print (f"[zhaoyukun02] loss in trainer.py, use_apex={self.use_apex}") #use_apex=False
        # print (f"[zhaoyukun02] loss in trainer.py, loss={loss}") #loss=3.0178470611572266
        # print (f"[zhaoyukun02] loss.shape={loss.shape}, gradient_acc_steps={self.args.gradient_accumulation_steps} in trainer")
        # # loss.shape=torch.Size([]), gradient_acc_steps=2
        if self.args.n_gpu > 1:
